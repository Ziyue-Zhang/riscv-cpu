...
{
   val data_r_IO = new MemPortIo()
   val data_w_IO = new MemPortIo()
   val inst_r_IO = new MemPortIo()
...
class DatPath() extends Module
{
   ...
   io.inst_r_IO.addr := pc_reg
   io.inst_r_IO.en := true.B
   val inst = Wire(UInt(32.W))
   inst := io.inst_r_IO.data
   ...
   // WB Mux
   io.data_r_IO.en := true.B
   io.data_r_IO.addr := alu_out
   wb_data := MuxCase(alu_out, Array(
                  (io.ctl.wb_sel === WB_ALU) -> alu_out,
                  (io.ctl.wb_sel === WB_MEM) -> io.data_r_IO.data, 
   ...
   io.data_w_IO.en := true.B
   io.data_w_IO.addr  := alu_out
   io.data_w_IO.data := rs2_data.asUInt()     
